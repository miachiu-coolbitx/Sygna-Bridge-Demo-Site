...
function styles() {
  return (
    gulp.src('src/sass/styles.sass') //入口文件
    ...
    .pipe(rename('styles.min.css'))
    .pipe(gulp.dest('assets/css'))
    .pipe(connect.reload()) //做完一個元件，再重新跑一次 (所見即所得)
  );
}
...
function watchTask(done) {
  gulp.watch('*.html', html); //逗號後面是 referenced function
  gulp.watch('src/sass/**/*.sass', styles); //『**』代表所有檔案及資料夾
